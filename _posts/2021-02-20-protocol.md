...
| Protease Inhibitor | | 终浓度1X，Lysis Buffer用，公用 |
| Enzymatics RNase Inhibitor | | PBSI和Lysis Buffer和NIB用，胡 |
| ~~BSA~~ | | PBSI用，已有 |
| ~~FA~~ | | 母液浓度37%，已有 |
| ~~Glycine~~ | | 已有 |
| ~~RNase-free water~~ | | 已有 |
| Nuclease-free water | | 可以用RNase-free water吗，待确定|
| Tris-HCl | | pH7.4，Lysis Buffer用，师兄有 |
| Tris|| pH7.5，ligation用；pH8.0，STE buffer用，师兄有 |
| DpnⅡ| | 师兄定新的 |
| NaCl | | Lysis Buffer用，胡 |
| EDTA | | Lysis Buffer用，胡 |
| EgTA | | dA wash用，梁俊师兄 |
| NP-40 | | Lysis Buffer用，终浓度0.2%，公用 |
| ~~Triton X-100~~ | | 已有 |
| SDS | | 公用 |
| 10X NEBuffer 3.1 | | 师兄有 |
| T4 DNA Ligase Buffer | NEB | 终浓度1X, 2X, 母液应该10X, 溶解blocking strand etc.用，师兄有 |
| T4 DNA Ligase | | 师兄有 |
| HpyCH4V | NEB, #R0620L, Ispwich, MA | 师兄已订货 |
| dA-tail reaction buffer | NEBNext dA-Tailing Module (NEB, #E6053L, Ispwich, MA) | 已订货 |
| Klenow Fragment | NEBNext dA-Tailing Module (NEB, #E6053L, Ispwich, MA) | 已订货 |
| dNTP | | RT用，胡 |
| RT primer with biotin tag | | RT用，胡 |
| PEG 6000 | | RT用，公用 |
| Maxima H Minus Reverse Transcriptase | | RT用，胡 |
| MgCl2 | | ligation用，许凯师兄有 |
...
| Hybridization mix | 1X T4 ligation buffer, 0.32 U/ml Enzymatics RNase Inhibitor, 0.05 U/ml SUPERase RI, 0.1% Triton X-100, and 0.25X NIB |
